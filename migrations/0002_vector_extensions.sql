...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        -- Create an index operator class for the cosine distance operator
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'embeddings_store' AND column_name = 'updated_at'
    ) THEN
        ALTER TABLE "embeddings_store" ADD COLUMN "updated_at" TIMESTAMP NOT NULL DEFAULT now();
    END IF;
END $$;

-- Create a GIN index on the vector field for faster similarity searches
CREATE INDEX IF NOT EXISTS "idx_embeddings_vector_gin" ON "embeddings_store" USING GIN ("vector");
