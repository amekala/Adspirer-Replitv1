    ...
    <div className="flex justify-start group py-2 chat-message assistant">
      <div className="flex flex-row gap-3 max-w-[85%] md:max-w-[75%]">
        <div className="flex-shrink-0 mr-2">
          <Avatar className="h-9 w-9 bg-indigo-600 dark:bg-zinc-800 self-start mt-0.5 overflow-hidden ring-2 ring-background">
            <div className="flex items-center justify-center h-full w-full">
        ...
        <Card className="p-0 overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm rounded-2xl rounded-tl-sm">
          ...
          {hasCampaignData && campaignData && campaignData.campaigns.length > 0 ? (
            <div className="p-4">
              ...
              {isStreaming && (
                <div className="mb-2">
                  <span className="inline-block animate-pulse text-slate-900 dark:text-slate-200">▌</span>
                </div>
              )}
            ...
            <MessageParser messageContent={message.content}>
              {(parsedData) => 
                parsedData.length > 0 ? (
                  ...
                  <div className="p-4 whitespace-pre-wrap text-sm md:text-base text-slate-800 dark:text-slate-200">
                    {message.content}
                    {isStreaming && <span className="inline-block animate-pulse ml-0.5 text-slate-900 dark:text-slate-200">▌</span>}
                  </div>
                )
              }
            </MessageParser>
