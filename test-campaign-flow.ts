 ...
 * 
 * Run with: ts-node test-campaign-flow.ts
 */
...
import { streamChatCompletion, generateWelcomeMessage, detectCampaignCreationIntent } from './server/services/openai';

// Mock Express Response class to capture streaming responses
...
const logError = (step: number, error: any) => {
  console.error(`\n${'!'.repeat(80)}`);
...
async function getAssistantResponse(conversationId: string, userId: string, userMessage: string) {
  try {
    ...
    const responseContent = (res as unknown as MockResponse).getContent();
    
    // Debug: log all chunks if there's an issue
    if (!responseContent) {
      console.error('Warning: Empty response content, showing all chunks:');
      (res as unknown as MockResponse).printAllChunks();
    }
